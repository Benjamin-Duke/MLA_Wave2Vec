...
class MaskingWithLearnableEmbedding(nn.Module):
    ...
    def forward(self, latent_reps, mask_prob, mask_length):
        ...
        mask = torch.ones(batch_size, seq_length, device=latent_reps.device)  # Initialiser un masque binaire (1 pour non masqué)
        mask_indices = torch.zeros(batch_size, seq_length, dtype=torch.long, device=latent_reps.device)  # Indices de masque
        
        for b in range(batch_size):
            indices = np.random.choice(seq_length, size=int(mask_prob * seq_length), replace=False)
            mask_indices[b, indices] = 1  # Marquer ces indices comme masqués
            for idx in indices:
                start = idx
                end = min(start + mask_length, seq_length)
                mask[b, start:end] = 0  # Mettre à 0 les indices masqués dans le masque binaire
            
        # Créer les représentations masquées
        masked_reps = latent_reps.clone()
        for b in range(batch_size):
            for idx in np.where(mask_indices[b].cpu().numpy())[0]:  # Utiliser les indices masqués
                start = idx
                end = min(start + mask_length, seq_length)
                masked_reps[b, start:end] = self.mask_embedding  # Remplacer les valeurs masquées par mask_embedding
        
        return masked_reps, mask, mask_indices  # Retourner mask_indices pour l'utiliser dans la fonction contrastive_loss
